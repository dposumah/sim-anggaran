    ...
    for (let i = 0; i < rawLines.length; i++) {
      ...
      if (lineText.includes('Spesifikasi :')) {
          ...
          if (prevLineObj && !prevLineObj.texts.map(t=>t.text).join(' ').match(/^(\[|^5\.\d+\.\d+\.\d+|Sumber|Sub Kegiatan|Spesifikasi)/)) {
             ...
             if (!hasLeftText) isNewUraian = true; // If prev line has NO left text, maybe it's just numbers from prev item? 
             // Wait, if it's the uraian line, it MUST have left text!
             if (hasLeftText) {
          ...
          if (extractedUraian.includes('PROVINSI SULAWESI UTARA')) {
              console.log('\n========================');
              console.log('URAIAN:', extractedUraian);
              console.log('SPESIFIKASI:', extractedSpesifikasi);
              console.log('JUMLAH MATCH:', jmlMatch ? jmlMatch[0] : 'FAILED');
          }
      }
    }
});
