...
pdfParser.on('pdfParser_dataReady', pdfData => {
    ...
    for (let i = 0; i < rawLines.length; i++) {
      ...
      if (lineText.includes('Spesifikasi :')) {
          ...
          // Sort all collected texts strictly by X, then Y, to simulate reading left-to-right, top-to-bottom?
          // Actually, if we just extract all currency matches:
          let fullString = allItemTexts.map(t => t.text).join(' ');
          let matches = fullString.match(/(\d{1,3}(?:\.\d{3})*,\d{2}|-)/g);
          
          let jumlah = 'NOT FOUND';
          if (matches && matches.length >= 2) {
             // The very last match is "Bertambah/(Berkurang)" (e.g., 0,00 or -54.000.000,00)
             // The second to last match is "Jumlah (Sesudah)"
             jumlah = matches[matches.length - 2];
          }
          
          console.log(lineText.substring(0, 30).padEnd(30) + ' => Matches: ' + (matches ? matches.length : 0) + ' | Jumlah: ' + jumlah);
      }
    }
});
