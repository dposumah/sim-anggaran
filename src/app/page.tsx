  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div 
          onClick={() => setModalData({ title: "Daftar Program", data: data?.programList || [] })}
          className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 cursor-pointer hover:border-primary/30 transition-all group"
        >
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-blue-50 text-blue-600 rounded-lg group-hover:bg-blue-100 transition-colors"><Building className="w-5 h-5" /></div>
            <h3 className="font-semibold text-gray-700">Jumlah Program</h3>
          </div>
          <p className="text-3xl font-bold text-gray-900">{data?.programList?.length || 0}</p>
        </div>
        
        <div 
          onClick={() => setModalData({ title: "Daftar Kegiatan", data: data?.kegiatanList || [] })}
          className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 cursor-pointer hover:border-primary/30 transition-all group"
        >
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-indigo-50 text-indigo-600 rounded-lg group-hover:bg-indigo-100 transition-colors"><Briefcase className="w-5 h-5" /></div>
            <h3 className="font-semibold text-gray-700">Jumlah Kegiatan</h3>
          </div>
          <p className="text-3xl font-bold text-gray-900">{data?.kegiatanList?.length || 0}</p>
        </div>

        <div 
          onClick={() => setModalData({ title: "Daftar Sub Kegiatan", data: data?.subKegiatanList || [] })}
          className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 cursor-pointer hover:border-primary/30 transition-all group"
        >
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-purple-50 text-purple-600 rounded-lg group-hover:bg-purple-100 transition-colors"><Package className="w-5 h-5" /></div>
            <h3 className="font-semibold text-gray-700">Jumlah Sub Kegiatan</h3>
          </div>
          <p className="text-3xl font-bold text-gray-900">{data?.subKegiatanList?.length || 0}</p>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[85vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-5 border-b border-gray-100 bg-gray-50/50">
              <h3 className="font-bold text-xl text-gray-800">{modalData.title}</h3>
              <button onClick={() => setModalData(null)} className="p-2 hover:bg-gray-200 rounded-full transition-colors">
                ...
                <table className="w-full text-sm text-left">
                  <thead className="bg-gray-50 text-gray-600 font-semibold border-b sticky top-0 shadow-sm z-10">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-right whitespace-nowrap">Pagu Induk</th>
                      <th className="px-4 py-3 text-right whitespace-nowrap">Pagu Perubahan</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {modalData.data.map((item, idx) => (
                      <tr key={idx} className="hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-3 font-mono text-xs text-gray-500 whitespace-nowrap">{item.kode}</td>
                        <td className="px-4 py-3 font-medium text-gray-800 min-w-[200px]">{item.nama}</td>
                        <td className="px-4 py-3 text-right">{formatCurrency(item.paguInduk)}</td>
                        <td className="px-4 py-3 text-right font-semibold text-primary">{formatCurrency(item.paguPerubahan)}</td>
                        <td className="px-4 py-3 text-right text-emerald-600">{formatCurrency(item.realisasi)}</td>
                      </tr>
                    ))}
                    {modalData.data.length === 0 && (
                      <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-500">Data kosong</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
