...
export default function LaporanPerbandingan() {
  ...
  const getChartDataForTabs = () => {
    ...
    return (data.allPaket || data.topPaket).map((p: any) => ({ ...p, paguInduk: p.induk, paguRkpd: p.rkpd, paguPerubahan: p.perubahan }));
  };
                ...
                <table className="min-w-full divide-y divide-gray-200 text-sm">
                  ...
                  <tbody className="divide-y divide-gray-200 bg-white">
                    {selectedPaket.rincian.length === 0 ? (
                      <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500">Belum ada rincian yang terdata</td></tr>
                    ) : (
